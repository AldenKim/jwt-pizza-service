...
class DB {
  constructor(dbName) {
    if (!dbName)
      throw new Error("Database name must be provided to DB constructor");
    this.dbName = dbName;
    this.isTest = dbName.toLowerCase().includes("test");
  ...
  async initializeDatabase() {
    try {
      ...
      try {
        if (this.isTest) {
          console.log(
            `Test environment detected. Dropping and recreating: ${dbName}`,
          );
  ...
  async dropDatabase() {
    if (!this.isTest) {
      console.warn(`Safety guard: dropDatabase skipped for non-test DB.`);
      return;
    }
    ...
    try {
      ...
      try {
        console.log(`Blowing away entire database: ${this.dbName}`);
...
const db = new DB(config.db.connection.database);
